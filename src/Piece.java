...
public class Piece {
    ...
    protected void goToDir(Piece[][] matrix, int[] direction) {
        int flag = 0;
        ...
        for (int i = 0; i < 7; i++) {
            if (Utils.withinMatrix(direction) && flag == 0) {
                ...
                if (value == null) {
                    legalMoves.add(direction.clone());
                    direction[0] += step[0];
                    direction[1] += step[1];
                } else {
    ...
    public ArrayList<int[]> getLegalMoves(Piece[][] matrix){ return new ArrayList<int[]>(); };
